...
// GET /api/social/tag?tag=push&page=0
export async function GET(request: NextRequest) {
  try {
    ...
    if (!tag || tag.length < 1) {
      return NextResponse.json({ data: null, error: 'tag requis' }, { status: 400 })
    }

    const offset = page * PAGE_SIZE

    // Recherche par ILIKE dans la caption (insensible à la casse, supporte accents)
    ...
    const feed = shares.map((share) => {
      ...
      const workout = (share.workouts as unknown) as {
        session_name: string | null; total_tonnage_kg: number | null
        total_sets: number | null; completed_at: string | null
      } | null

      return {
        id: share.id, workout_id: share.workout_id, user_id: share.user_id,
        caption: share.caption, likes_count: share.likes_count ?? 0, comments_count: share.comments_count ?? 0,
        created_at: share.created_at, is_liked: likedIds.has(share.id),
        ...
        workout: workout ? {
          session_name: workout.session_name, total_tonnage_kg: workout.total_tonnage_kg,
          total_sets: workout.total_sets, completed_at: workout.completed_at,
        } : null,
      }
    })
