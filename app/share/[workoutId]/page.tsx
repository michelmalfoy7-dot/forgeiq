...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const desc = workout.total_tonnage_kg
    ? `${fmt(workout.total_tonnage_kg)} soulevés · ${workout.total_sets ?? 0} séries`
    : 'Séance complétée sur ForgeIQ'

  return {
    title,
    description: desc,
    openGraph: { title, description: desc, url: `${APP_URL}/share/${workoutId}`, type: 'website' },
...
export default async function ShareWorkoutPage({ params }: PageProps) {
  ...
  // Profil privé ou inexistant → page générique sans données perso
  ...
  // Top sets par exercice
  const { data: sets } = await supabaseAdmin
    .from('workout_sets')
    .select('exercise_name, weight_kg, reps, set_type')
  ...
  // Grouper par exercice → top set
  type ExTop = { name: string; kg: number; reps: number }
  const exMap = new Map<string, ExTop>()
  for (const s of sets ?? []) {
    const existing = exMap.get(s.exercise_name)
    if (!existing || s.weight_kg > existing.kg) {
      exMap.set(s.exercise_name, { name: s.exercise_name, kg: s.weight_kg, reps: s.reps })
    }
  }
  const topExercises = [...exMap.values()].slice(0, 5)
              ...
              <div key={i} className="flex items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-1.5 flex-shrink-0">
                  <span
                    ...
                    style={{ color: 'var(--fiq-accent)', fontVariantNumeric: 'tabular-nums' }}
                  >
                    {ex.kg}kg
                  </span>
