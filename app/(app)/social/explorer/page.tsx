...
export default async function ExplorerPage() {
  ...
  // Profils sociaux des auteurs du feed global
  const authorIds = [...new Set((trendingShares ?? []).map((s: { user_id: string }) => s.user_id))]
  const [{ data: socialProfiles }, { data: authProfiles }, { data: userLikes }] = await Promise.all([
    ...
    (trendingShares ?? []).length > 0
      ? supabase
          ...
          .eq('user_id', user.id)
          .in('workout_share_id', (trendingShares ?? []).map((s: { id: string }) => s.id))
      : Promise.resolve({ data: [] }),
  ])

  // Exercices pour le feed global
  const workoutIds = (trendingShares ?? []).map((s: { workout_id: string }) => s.workout_id).filter(Boolean)
  ...
  const trendingFeed: FeedPost[] = (trendingShares ?? []).map((share) => {
